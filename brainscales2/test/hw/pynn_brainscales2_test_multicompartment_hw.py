...
class BaseTestCases:
    class TestRecordingAndProjections(unittest.TestCase):
        ...
        compartment after another. Assert that the EPSP at the injection site
        is the largest.
        '''
        ...
        def get_psp_heights(signal: neo.IrregularlySampledSignal, n_inputs: int
            ...
            :param n_inputs: Number of inputs. The signal is cut in n_inputs
                equal segments and in each segment the PSP height is
                calculated.
            ...
            p_per_input = len(voltage) // n_inputs

            psp_heights = []
            for i in range(n_inputs):
                cut_v = voltage[i * p_per_input:(i + 1) * p_per_input]
        ...
        def test_recording_and_projections(self):
            ...
            neuron_class = self.create_neuron()
            pop = pynn.Population(1, neuron_class())

            isi = 0.5   # ms (hw)
            spikes = (np.arange(len(self.labels)) + 0.5) * isi

            for label, spike in zip(self.labels, spikes):
            ...
            for label in self.labels:
                ...
                pynn.run(isi * len(self.labels))

                data = pop.get_data(clear=True).segments[-1]\
                    .irregularlysampledsignals[-1]
                psp_heights.append(
                    self.get_psp_heights(data, len(self.labels)))
                pynn.reset()
        ...
        def test_cadc_recording(self):
            ...
            runtime = 0.15
            spikes = np.linspace(0.01, runtime - 0.05, len(self.labels))
        ...
        def test_madc_recording(self):
            ...
            labels_to_record = self.labels[:2]
            ...
            # MADC might record different number of samples for different
            # channels. If the number of samples is different, we already
        ...
        def test_readout_source(self):
            ...
            pynn.Projection(in_pop, pop, pynn.AllToAllConnector(),
                            ...
                            receptor_type=self.receptor_type)
            pop.record("v", locations=[self.labels[0]])
            pynn.run(1)
            pynn.reset()
            ...
            pop.record(None)
            pop.record("exc_synin", locations=[self.labels[0]])
            pynn.run(1)
            samples = pop.get_data().segments[-1].irregularlysampledsignals[0]
...
class TestRecordingAndProjectionsManual(
    ...
    def create_neuron(self):
        ...
        comps.append(PlacedCompartment(positions=[1, 2], label=self.labels[1],
                                       ...
                                       threshold_enable=False))
        comps.append(PlacedCompartment(positions=[3, 4], label=self.labels[2],
